...
describe("buildAiForecast", () => {
  it("returns eight forecast points, six actual history points, and clamps selected month into range", () => {
    ...
    expect(result.timeline[0]?.phase).toBe("actual");
    expect(result.timeline[5]?.label).toBe("当前");
    expect(result.timeline[6]?.phase).toBe("forecast");
  });
  ...
  it("changes projected outcome when target price, strategy, and basis assumptions change", () => {
    const base = buildAiForecast("CP-PK-240418-A1", 4, 14.2, "steady", -0.6);
    const optimistic = buildAiForecast("CP-PK-240418-A1", 4, 16.8, "aggressive", 1.2);
    ...
    expect(optimistic.targetPrice).toBeGreaterThan(base.targetPrice);
  });
});
