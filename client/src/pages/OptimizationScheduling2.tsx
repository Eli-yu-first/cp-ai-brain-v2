...
import type { OptimizationScheduling2TuningInput } from "@shared/optimizationScheduling2";
...
const DEFAULT_TUNING: OptimizationScheduling2TuningInput = {
...
const SLIDER_CONFIG: Array<{
  key: keyof OptimizationScheduling2TuningInput;
  label: string;
  min: number;
  max: number;
  step: number;
...
export default function OptimizationScheduling2Page() {
  ...
  const [tuning, setTuning] = useState<OptimizationScheduling2TuningInput>(DEFAULT_TUNING);
  ...
  const { data: result, isLoading, refetch } = trpc.platform.optimizationScheduling2Simulate.useQuery(
    { tuning: { ...tuning, partPriceAdjustments: tuning.partPriceAdjustments as Record<string, number> | undefined } },
    { refetchOnWindowFocus: false }
  );

  const chatMutation = trpc.platform.optimizationScheduling2Chat.useMutation();
  ...
  const handleTuningChange = useCallback((key: keyof OptimizationScheduling2TuningInput, value: number) => {
