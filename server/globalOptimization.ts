...
function buildYieldTree(yieldRates: YieldRateRow[]): Map<string, Array<{ child: string; rate: number; process: number }>> {
  ...
  for (const row of yieldRates) {
    ...
    children.push({ child: row.childMaterial, rate: row.yieldRate, process: typeof row.process === "number" ? row.process : Number(row.process) });
...
function computePigCostAllocation(
  ...
  const splitCostByFactoryPart = new Map<string, SplitCostRow>();
  for (const sc of splitCosts) {
    splitCostByFactoryPart.set(`${sc.factoryId}_${sc.part}`, sc);
  }
  ...
  for (const profit of profitResults) {
    ...
    const splitCostRow = splitCostByFactoryPart.get(`${profit.factoryId}_${profit.part}`);
    const slaughterCost = profit.salesKg * (splitCostRow?.slaughterCostPerKg ?? 0);
    const splitCost = profit.salesKg * (splitCostRow?.splitCostPerKg ?? 0);
    const packageCost = profit.salesKg * (splitCostRow?.packageCostPerKg ?? 0);
    const freezeCost = profit.salesKg * (splitCostRow?.freezeCostPerKg ?? splitCostRow?.freezePackCostPerKg ?? 0);
    const processingCost = profit.salesKg * (splitCostRow?.costCoefficient ?? 0);
    ...
    profit.profit = Math.round((profit.revenue - profit.pigCost - profit.storageCost - profit.transportCost - profit.slaughterCost - profit.splitCost - profit.packageCost - profit.freezeCost - profit.processingCost) * 100) / 100;
  }
