...
| 输出字段 | 计算公式 |
| --- | --- |
| 建议合约手数 | `CEILING(Q × H / C, 1)` |
| 对冲后预期损益元 | `现货损益 + 期货损益 - 默认成本` |
| 保证金占用元 | `F0 × 1000 × 实际套保吨数 × M` |
| 套保有效性 | `1 - ABS(现货损益 + 期货损益) / ABS(现货损益)` |
| 推荐动作 | 根据损益、有效性、保证金占用判断 |

**逐步计算**

```text
1. 建议套保吨数 = 现货敞口量吨 × 计划套保比例
   HedgeTon = Q × H

2. 建议合约手数 = 向上取整(建议套保吨数 / 合约规模)
   Contracts = CEILING(HedgeTon / 16, 1)

3. 实际套保吨数 = 建议合约手数 × 合约规模
   ActualHedgeTon = Contracts × 16
```

现货损益：

```text
现货损益 = (预期未来现货价 - 当前现货价) × 现货敞口量吨 × 1000

SpotPnL = (S1 - S0) × Q × 1000
```

期货损益，按“卖出期货套保”计算：

```text
期货损益 = (当前期货价 - 预期未来期货价) × 实际套保吨数 × 1000
```
...
```text
FuturesPnL = (F0 - S1) × ActualHedgeTon × 1000
```
...
```text
仓储成本 = W × D × Q
融资成本 = S0 × Q × 1000 × R × D / 365
交易成本 = T × ActualHedgeTon
滑点成本 = L × ActualHedgeTon × 1000
...
```text
套保有效性 =
1 - ABS(SpotPnL + FuturesPnL) / ABS(SpotPnL)
```

建议做保护：

```text
如果 SpotPnL = 0，则套保有效性记为 100% 或“不适用”
如果套保有效性 < 0，则记为 0%
如果套保有效性 > 100%，则记为 100%
```
...
```text
如果 对冲后预期损益 > 0 且 套保有效性 >= 60%
推荐动作 = 建仓

如果 对冲后预期损益 > 0 但 套保有效性 < 60%
推荐动作 = 降低比例/重新测算

如果 对冲后预期损益 <= 0 且 当前期货价 > 当前现货价
推荐动作 = 观望
...
R2 套保有效性
=IF(I2=0,"不适用",MAX(0,MIN(100%,1-ABS(I2+J2)/ABS(I2))))

S2 推荐动作
=IF(Q2>3000000,"需审批",IF(AND(P2>0,R2>=60%),"建仓",IF(AND(P2>0,R2<60%),"降低比例/重新测算",IF(D2>C2,"观望","不建议对冲"))))
```

如果你要保持“输出只有 5 个字段”，可以隐藏中间计算列 `H:O`，只显示：
